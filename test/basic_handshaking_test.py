...
def normal_session():
    ...
    handshaking_session.add_peer(
        1,
        "src/peer.py",
        "test/tmp1/nodes1.map",
        "test/tmp1/data1.fragment",
        1,
        ("127.0.0.1", 48001),
    )
    handshaking_session.add_peer(
...
def test_flooding_whohas(normal_session):
    ...
    for i in range(48002, 48008):
        assert (
            handshaking_session.peer_list[("127.0.0.1", 48001)].send_record[
                ("127.0.0.1", i)
            ][0]
            > 0
...
def test_send_ihave(normal_session):
    handshaking_session = normal_session
    assert (
        handshaking_session.peer_list[("127.0.0.1", 48002)].send_record[
            ("127.0.0.1", 48001)
        ][1]
        > 0
    ), "Fail to send IHAVE"
...
def test_handshaking(normal_session):
    handshaking_session = normal_session
    assert (
        handshaking_session.peer_list[("127.0.0.1", 48002)].recv_record[
            ("127.0.0.1", 48001)
        ][0]
        > 0
    ), "Fail to receive any WHOHAS"
    assert (
        handshaking_session.peer_list[("127.0.0.1", 48001)].recv_record[
            ("127.0.0.1", 48002)
        ][1]
        > 0
    ), "Fail to receive any IHAVE"
    assert (
        handshaking_session.peer_list[("127.0.0.1", 48002)].recv_record[
            ("127.0.0.1", 48001)
        ][2]
        > 0
    ), "Fail to receive any DOWNLOAD"
    assert (
        handshaking_session.peer_list[("127.0.0.1", 48001)].recv_record[
            ("127.0.0.1", 48002)
        ][3]
