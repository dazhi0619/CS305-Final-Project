...
def concurrent_session():
    ...
    stime = time.time()
    concurrent_session = grader.GradingSession(
        grader.normal_handler, latency=0.01, spiffy=True
    )
    concurrent_session.add_peer(
        1,
        "src/peer.py",
        "test/tmp3/nodes3.map",
        "test/tmp3/data3-1.fragment",
        100,
        ("127.0.0.1", 48001),
    )
    concurrent_session.add_peer(
    ...
    concurrent_session.add_peer(
        ...
        "src/peer.py",
        "test/tmp3/nodes3.map",
        "test/tmp3/data3-3.fragment",
        100,
        ("127.0.0.1", 48003),
    )
    concurrent_session.run_grader()

    concurrent_session.peer_list[("127.0.0.1", 48001)].send_cmd(
        """DOWNLOAD test/tmp3/download_target3.chunkhash test/tmp3/download_result.fragment\n"""
    )
